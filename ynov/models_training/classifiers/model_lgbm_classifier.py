...
#
# Classes :
# - ModelLGBMClassifier -> Modèle pour prédictions via Light GBM - Classification
...
class ModelLGBMClassifier(ModelClassifierMixin, ModelPipeline):
    '''Modèle pour prédictions via Light GBM - Classification'''

    _default_name = 'model_lgbm_classifier'

    def __init__(self, lgbm_params: dict = {}, multiclass_strategy: str = None, **kwargs):
        '''Initialisation de la classe (voir ModelPipeline, ModelClass & ModelClassifierMixin pour arguments supplémentaires)

        Kwargs:
            lgbm_params (dict) : paramètres pour le Light GBM
        ...
        # Le LGBMClassifier ne supporte pas nativement le multi_label
        if self.multi_label:
