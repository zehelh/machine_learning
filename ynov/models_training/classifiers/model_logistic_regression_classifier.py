...
#
# Classes :
# - ModelLogisticRegressionClassifier -> Modèle pour prédictions via Logistic Regression - Classification
...
class ModelLogisticRegressionClassifier(ModelClassifierMixin, ModelPipeline):
    '''Modèle pour prédictions via Logistic Regression - Classification'''

    _default_name = 'model_lr_classifier'

    def __init__(self, lr_params: dict = {}, multiclass_strategy: str = None, **kwargs):
        '''Initialisation de la classe (voir ModelPipeline, ModelClass & ModelClassifierMixin pour arguments supplémentaires)

        Kwargs:
            lr_params (dict) : paramètres pour le Logistic Regression
        ...
        # Le LogisticRegression ne supporte pas nativement le multi_label
        if self.multi_label:
