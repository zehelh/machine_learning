...
## Modèle Gradient Boosted Tree
# Auteurs : Agence dataservices
# Date : 28/10/2019
#
# Classes :
# - ModelGBTClassifier -> Modèle pour prédictions via Gradient Boosted Tree - Classification
...
class ModelGBTClassifier(ModelClassifierMixin, ModelPipeline):
    '''Modèle pour prédictions via Gradient Boosted Tree - Classification'''

    _default_name = 'model_gbt_classifier'

    def __init__(self, gbt_params: dict = {}, multiclass_strategy: str = None, **kwargs):
        '''Initialisation de la classe (voir ModelPipeline, ModelClass & ModelClassifierMixin pour arguments supplémentaires)

        Kwargs:
            gbt_params (dict) : paramètres pour le Gradient Boosted Tree
            multiclass_strategy (str): stratégie multiclass, 'ovr' (OneVsRest), ou 'ovo' (OneVsOne). Si None, on laisse l'algo tel quel.
                Attention, 'ovo' ne peut pas prédire de probas. Par défaut on retourne 1. si classe pred, sinon 0.
        Raises:
        ...
        # Le GradientBoostingClassifier ne supporte pas nativement le multi_label
        if self.multi_label:
