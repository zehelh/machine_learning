...
from ynov.models_training.regressors.model_kernel_ridge_regressor import ModelKernelRidgeRegressor
...
class ModelKernelRidgeRegressorTests(unittest.TestCase):
    '''Main class to test model_kernel_ridge_regressor'''
    ...
    def test01_model_kernel_ridge_regressor_init(self):
        '''Test de l'initialisation de ynov.models_training.model_kernel_ridge_regressor.ModelKernelRidgeRegressor'''

        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)

        # Init., test all params
        model = ModelKernelRidgeRegressor(model_dir=model_dir)
        ...
        # Check KernelRidge params
        model = ModelKernelRidgeRegressor(model_dir=model_dir, kernel_ridge_params={'alpha': 0.5, 'kernel': 'linear'})
        self.assertEqual(model.pipeline['kernel_ridge'].alpha, 0.5)
        self.assertEqual(model.pipeline['kernel_ridge'].kernel, 'linear')
        remove_dir(model_dir)

    def test02_model_kernel_ridge_regressor_predict(self):
        '''Test de la fonction predict de ynov.models_training.model_kernel_ridge_regressor.ModelKernelRidgeRegressor'''
        ...
        # Regressor
        model = ModelKernelRidgeRegressor(x_col=x_col, y_col=y_col_mono, model_dir=model_dir)
    ...
    def test03_model_kernel_ridge_regressor_save(self):
        '''Test de la fonction save de ynov.models_training.model_kernel_ridge_regressor.ModelKernelRidgeRegressor'''
        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)
        ...
        self.assertTrue('kernel_ridge_confs' in configs.keys())
        remove_dir(model_dir)

    def test04_model_kernel_ridge_regressor_reload_from_standalone(self):
        '''Test de la fonction ynov.models_training.regressors.model_kernel_ridge_regressor.ModelKernelRidgeRegressor.reload_from_standalone'''
        ...
        new_model = ModelKernelRidgeRegressor()
        ...
        self.assertEqual(model.kernel_ridge.get_params(), kernel_ridge.get_params())
