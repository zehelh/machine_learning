...
from ynov.models_training.regressors.model_knn_regressor import ModelKNNRegressor
...
class ModelKNNRegressorTests(unittest.TestCase):
    '''Main class to test model_knn_regressor'''
    ...
    def test01_model_knn_regressor_init(self):
        '''Test de l'initialisation de ynov.models_training.model_knn_regressor.ModelKNNRegressor'''

        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)

        # Init., test all params
        model = ModelKNNRegressor(model_dir=model_dir)
        ...
        # Check KNN params
        model = ModelKNNRegressor(model_dir=model_dir, knn_params={'n_neighbors': 7, 'weights': 'distance'})
        self.assertEqual(model.pipeline['knn'].n_neighbors, 7)
        self.assertEqual(model.pipeline['knn'].weights, 'distance')
        remove_dir(model_dir)

    def test02_model_knn_regressor_predict(self):
        '''Test de la fonction predict de ynov.models_training.model_knn_regressor.ModelKNNRegressor'''
        ...
        # Regressor
        model = ModelKNNRegressor(x_col=x_col, y_col=y_col_mono, model_dir=model_dir)
    ...
    def test03_model_knn_regressor_save(self):
        '''Test de la fonction save de ynov.models_training.model_knn_regressor.ModelKNNRegressor'''
        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)
    ...
    def test04_model_knn_regressor_reload_from_standalone(self):
        '''Test de la fonction ynov.models_training.regressors.model_knn_regressor.ModelKNNRegressor.reload_from_standalone'''
