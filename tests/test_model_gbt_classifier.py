...
from ynov.models_training.classifiers.model_gbt_classifier import ModelGBTClassifier
...
class ModelGBTClassifierTests(unittest.TestCase):
    '''Main class to test model_gbt_classifier'''
    ...
    def test01_model_gbt_classifier_init(self):
        '''Test de ynov.models_training.classifiers.model_gbt_classifier.ModelGBTClassifier.__init__'''

        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)

        # Init., test all params
        model = ModelGBTClassifier(model_dir=model_dir)
        ...
        # Check GBT params en fonction multi-label & multiclass strategy
        model = ModelGBTClassifier(model_dir=model_dir, multi_label=False, gbt_params={'learning_rate': 0.5, 'n_estimators': 10})
        self.assertEqual(model.pipeline['gbt'].learning_rate, 0.5)
        self.assertEqual(model.pipeline['gbt'].n_estimators, 10)
        remove_dir(model_dir)
        model = ModelGBTClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='ovr', gbt_params={'learning_rate': 0.5, 'n_estimators': 10})
        ...
        remove_dir(model_dir)
        model = ModelGBTClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='ovo', gbt_params={'learning_rate': 0.5, 'n_estimators': 10})
        self.assertEqual(model.multiclass_strategy, 'ovo')
        self.assertEqual(model.pipeline['gbt'].estimator.learning_rate, 0.5)
        self.assertEqual(model.pipeline['gbt'].estimator.n_estimators, 10)
        remove_dir(model_dir)
        #
        model = ModelGBTClassifier(model_dir=model_dir, multi_label=True, gbt_params={'learning_rate': 0.5, 'n_estimators': 10})
        self.assertEqual(model.pipeline['gbt'].estimator.learning_rate, 0.5)
        self.assertEqual(model.pipeline['gbt'].estimator.n_estimators, 10)
        self.assertEqual(model.multi_label, True)
        remove_dir(model_dir)
        model = ModelGBTClassifier(model_dir=model_dir, multi_label=True, multiclass_strategy='ovr', gbt_params={'learning_rate': 0.5, 'n_estimators': 10})
        self.assertEqual(model.multiclass_strategy, 'ovr')
        self.assertEqual(model.pipeline['gbt'].estimator.learning_rate, 0.5)
        self.assertEqual(model.pipeline['gbt'].estimator.n_estimators, 10)
        self.assertEqual(model.multi_label, True)
        ...
        with self.assertRaises(ValueError):
            model = ModelGBTClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='toto', gbt_params={'learning_rate': 0.5, 'n_estimators': 10})
        remove_dir(model_dir)

    def test02_model_gbt_classifier_predict(self):
        '''Test de la fonction predict de ynov.models_training.classifiers.model_gbt_classifier.ModelGBTClassifier'''
        ...
        model = ModelGBTClassifier(x_col=x_col, y_col=y_col_mono, model_dir=model_dir)
        ...
        remove_dir(model_dir)
        model = ModelGBTClassifier(x_col=x_col, y_col=y_col_mono, model_dir=model_dir, multiclass_strategy='ovr')
        model.fit(x_train, y_train_mono_2)
        preds = model.predict(x_train, return_proba=False)
        self.assertEqual(preds.shape, (len(x_train),))
        proba = model.predict(x_train, return_proba=True)
        self.assertEqual(proba.shape, (len(x_train), 2)) # 2 classes
        remove_dir(model_dir)
        model = ModelGBTClassifier(x_col=x_col, y_col=y_col_mono, model_dir=model_dir, multiclass_strategy='ovo')
        model.fit(x_train, y_train_mono_2)
        preds = model.predict(x_train, return_proba=False)
        self.assertEqual(preds.shape, (len(x_train),))
        proba = model.predict(x_train, return_proba=True)
        ...
        model = ModelGBTClassifier(x_col=x_col, y_col=y_col_multi, model_dir=model_dir, multi_label=True)
        ...
        remove_dir(model_dir)
        model = ModelGBTClassifier(x_col=x_col, y_col=y_col_multi, model_dir=model_dir, multi_label=True, multiclass_strategy='ovr')
        model.fit(x_train, y_train_multi)
        preds = model.predict(x_train)
        self.assertEqual(preds.shape, (len(x_train), len(y_col_multi)))
        proba = model.predict(x_train, return_proba=True)
        self.assertEqual(proba.shape, (len(x_train), len(y_col_multi)))
        remove_dir(model_dir)
        model = ModelGBTClassifier(x_col=x_col, y_col=y_col_multi, model_dir=model_dir, multi_label=True, multiclass_strategy='ovo')
        model.fit(x_train, y_train_multi)
        preds = model.predict(x_train)
        self.assertEqual(preds.shape, (len(x_train), len(y_col_multi)))
        proba = model.predict(x_train, return_proba=True)
    ...
    def test03_model_gbt_classifier_predict_proba(self):
        '''Test de la fonction predict_proba de ynov.models_training.classifiers.model_gbt_classifier.ModelGBTClassifier'''
    ...
    def test04_model_gbt_classifier_get_predict_position(self):
        '''Test de la fonction ynov.models_training.classifiers.model_gbt_classifier.ModelGBTClassifier.get_predict_position'''
    ...
    def test05_model_gbt_classifier_save(self):
        '''Test de la fonction save de ynov.models_training.classifiers.model_gbt_classifier.ModelGBTClassifier'''

        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)

        # Fonctionnement nominal
        model = ModelGBTClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='ovr')
    ...
    def test06_model_gbt_classifier_reload_from_standalone(self):
        '''Test de la fonction ynov.models_training.classifiers.model_gbt_classifier.ModelGBTClassifier.reload_from_standalone'''
