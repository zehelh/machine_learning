...
from ynov.models_training.classifiers.model_svm_classifier import ModelSVMClassifier
...
class ModelSVMClassifierTests(unittest.TestCase):
    '''Main class to test model_svm_classifier'''
    ...
    def test01_model_svm_classifier_init(self):
        '''Test de ynov.models_training.classifiers.model_svm_classifier.ModelSVMClassifier.__init__'''

        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)

        # Init., test all params
        model = ModelSVMClassifier(model_dir=model_dir)
        ...
        # Check SVM params en fonction multi-label & multiclass strategy
        model = ModelSVMClassifier(model_dir=model_dir, multi_label=False, svm_params={'C': 0.5, 'kernel': 'linear'})
        self.assertEqual(model.pipeline['svm'].C, 0.5)
        self.assertEqual(model.pipeline['svm'].kernel, 'linear')
        remove_dir(model_dir)
        model = ModelSVMClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='ovr', svm_params={'C': 0.5, 'kernel': 'linear'})
        self.assertEqual(model.multiclass_strategy, 'ovr')
        self.assertEqual(model.pipeline['svm'].estimator.C, 0.5)
        self.assertEqual(model.pipeline['svm'].estimator.kernel, 'linear')
        remove_dir(model_dir)
        model = ModelSVMClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='ovo', svm_params={'C': 0.5, 'kernel': 'linear'})
        self.assertEqual(model.multiclass_strategy, 'ovo')
        self.assertEqual(model.pipeline['svm'].estimator.C, 0.5)
        self.assertEqual(model.pipeline['svm'].estimator.kernel, 'linear')
        remove_dir(model_dir)
        #
        model = ModelSVMClassifier(model_dir=model_dir, multi_label=True, svm_params={'C': 0.5, 'kernel': 'linear'})
        self.assertEqual(model.pipeline['svm'].estimator.C, 0.5)
        self.assertEqual(model.pipeline['svm'].estimator.kernel, 'linear')
        self.assertEqual(model.multi_label, True)
        remove_dir(model_dir)
        model = ModelSVMClassifier(model_dir=model_dir, multi_label=True, multiclass_strategy='ovr', svm_params={'C': 0.5, 'kernel': 'linear'})
        self.assertEqual(model.multiclass_strategy, 'ovr')
        self.assertEqual(model.pipeline['svm'].estimator.C, 0.5)
        self.assertEqual(model.pipeline['svm'].estimator.kernel, 'linear')
        self.assertEqual(model.multi_label, True)
        remove_dir(model_dir)
        model = ModelSVMClassifier(model_dir=model_dir, multi_label=True, multiclass_strategy='ovo', svm_params={'C': 0.5, 'kernel': 'linear'})
        self.assertEqual(model.multiclass_strategy, 'ovo')
        self.assertEqual(model.pipeline['svm'].estimator.C, 0.5)
        self.assertEqual(model.pipeline['svm'].estimator.kernel, 'linear')
        self.assertEqual(model.multi_label, True)
        ...
        with self.assertRaises(ValueError):
            model = ModelSVMClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='toto', svm_params={'C': 0.5, 'kernel': 'linear'})
        remove_dir(model_dir)

    def test02_model_svm_classifier_predict(self):
        '''Test de la fonction predict de ynov.models_training.classifiers.model_svm_classifier.ModelSVMClassifier'''
        ...
        model = ModelSVMClassifier(x_col=x_col, y_col=y_col_mono, model_dir=model_dir)
        ...
        remove_dir(model_dir)
        model = ModelSVMClassifier(x_col=x_col, y_col=y_col_mono, model_dir=model_dir, multiclass_strategy='ovr')
        model.fit(x_train, y_train_mono_2)
        preds = model.predict(x_train, return_proba=False)
        self.assertEqual(preds.shape, (len(x_train),))
        proba = model.predict(x_train, return_proba=True)
        self.assertEqual(proba.shape, (len(x_train), 2)) # 2 classes
        remove_dir(model_dir)
        model = ModelSVMClassifier(x_col=x_col, y_col=y_col_mono, model_dir=model_dir, multiclass_strategy='ovo')
        model.fit(x_train, y_train_mono_2)
        preds = model.predict(x_train, return_proba=False)
        self.assertEqual(preds.shape, (len(x_train),))
        proba = model.predict(x_train, return_proba=True)
        ...
        model = ModelSVMClassifier(x_col=x_col, y_col=y_col_multi, model_dir=model_dir, multi_label=True)
        ...
        remove_dir(model_dir)
        model = ModelSVMClassifier(x_col=x_col, y_col=y_col_multi, model_dir=model_dir, multi_label=True, multiclass_strategy='ovr')
        model.fit(x_train, y_train_multi)
        preds = model.predict(x_train)
        self.assertEqual(preds.shape, (len(x_train), len(y_col_multi)))
        proba = model.predict(x_train, return_proba=True)
        self.assertEqual(proba.shape, (len(x_train), len(y_col_multi)))
        remove_dir(model_dir)
        model = ModelSVMClassifier(x_col=x_col, y_col=y_col_multi, model_dir=model_dir, multi_label=True, multiclass_strategy='ovo')
        model.fit(x_train, y_train_multi)
        preds = model.predict(x_train)
        self.assertEqual(preds.shape, (len(x_train), len(y_col_multi)))
        proba = model.predict(x_train, return_proba=True)
    ...
    def test03_model_svm_classifier_predict_proba(self):
        '''Test de la fonction predict_proba de ynov.models_training.classifiers.model_svm_classifier.ModelSVMClassifier'''
    ...
    def test04_model_svm_classifier_get_predict_position(self):
        '''Test de la fonction ynov.models_training.classifiers.model_svm_classifier.ModelSVMClassifier.get_predict_position'''
    ...
    def test05_model_svm_classifier_save(self):
        '''Test de la fonction save de ynov.models_training.classifiers.model_svm_classifier.ModelSVMClassifier'''

        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)

        # Fonctionnement nominal
        model = ModelSVMClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='ovr')
        ...
        self.assertTrue('svm_confs' in configs.keys())
        remove_dir(model_dir)

    def test06_model_svm_classifier_reload_from_standalone(self):
        '''Test de la fonction ynov.models_training.classifiers.model_svm_classifier.ModelSVMClassifier.reload_from_standalone'''
        ...
        self.assertEqual(model.svm.get_params(), svm.get_params())
