...
from ynov.models_training.classifiers.model_logistic_regression_classifier import ModelLogisticRegressionClassifier
...
class ModelLogisticRegressionClassifierTests(unittest.TestCase):
    '''Main class to test model_logistic_regression_classifier'''
    ...
    def test01_model_logistic_regression_classifier_init(self):
        '''Test de ynov.models_training.classifiers.model_logistic_regression_classifier.ModelLogisticRegressionClassifier.__init__'''

        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)

        # Init., test all params
        model = ModelLogisticRegressionClassifier(model_dir=model_dir)
        ...
        # Check Logistic Regression params en fonction multi-label & multiclass strategy
        model = ModelLogisticRegressionClassifier(model_dir=model_dir, multi_label=False, lr_params={'C': 0.5, 'max_iter': 10})
        self.assertEqual(model.pipeline['lr'].C, 0.5)
        self.assertEqual(model.pipeline['lr'].max_iter, 10)
        remove_dir(model_dir)
        model = ModelLogisticRegressionClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='ovr', lr_params={'C': 0.5, 'max_iter': 10})
        self.assertEqual(model.multiclass_strategy, 'ovr')
        self.assertEqual(model.pipeline['lr'].estimator.C, 0.5)
        self.assertEqual(model.pipeline['lr'].estimator.max_iter, 10)
        remove_dir(model_dir)
        model = ModelLogisticRegressionClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='ovo', lr_params={'C': 0.5, 'max_iter': 10})
        self.assertEqual(model.multiclass_strategy, 'ovo')
        self.assertEqual(model.pipeline['lr'].estimator.C, 0.5)
        self.assertEqual(model.pipeline['lr'].estimator.max_iter, 10)
        remove_dir(model_dir)
        #
        model = ModelLogisticRegressionClassifier(model_dir=model_dir, multi_label=True, lr_params={'C': 0.5, 'max_iter': 10})
        self.assertEqual(model.pipeline['lr'].estimator.C, 0.5)
        self.assertEqual(model.pipeline['lr'].estimator.max_iter, 10)
        self.assertEqual(model.multi_label, True)
        remove_dir(model_dir)
        model = ModelLogisticRegressionClassifier(model_dir=model_dir, multi_label=True, multiclass_strategy='ovr', lr_params={'C': 0.5, 'max_iter': 10})
        self.assertEqual(model.multiclass_strategy, 'ovr')
        self.assertEqual(model.pipeline['lr'].estimator.C, 0.5)
        self.assertEqual(model.pipeline['lr'].estimator.max_iter, 10)
        self.assertEqual(model.multi_label, True)
        remove_dir(model_dir)
        model = ModelLogisticRegressionClassifier(model_dir=model_dir, multi_label=True, multiclass_strategy='ovo', lr_params={'C': 0.5, 'max_iter': 10})
        self.assertEqual(model.multiclass_strategy, 'ovo')
        self.assertEqual(model.pipeline['lr'].estimator.C, 0.5)
        self.assertEqual(model.pipeline['lr'].estimator.max_iter, 10)
        self.assertEqual(model.multi_label, True)
        ...
        with self.assertRaises(ValueError):
            model = ModelLogisticRegressionClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='toto', lr_params={'C': 0.5, 'max_iter': 10})
        remove_dir(model_dir)

    def test02_model_logistic_regression_classifier_predict(self):
        '''Test de la fonction predict de ynov.models_training.classifiers.model_logistic_regression_classifier.ModelLogisticRegressionClassifier'''
    ...
    def test03_model_logistic_regression_classifier_predict_proba(self):
        '''Test de la fonction predict_proba de ynov.models_training.classifiers.model_logistic_regression_classifier.ModelLogisticRegressionClassifier'''
    ...
    def test04_model_logistic_regression_classifier_get_predict_position(self):
        '''Test de la fonction ynov.models_training.classifiers.model_logistic_regression_classifier.ModelLogisticRegressionClassifier.get_predict_position'''
    ...
    def test05_model_logistic_regression_classifier_save(self):
        '''Test de la fonction save de ynov.models_training.classifiers.model_logistic_regression_classifier.ModelLogisticRegressionClassifier'''

        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)

        # Fonctionnement nominal
        model = ModelLogisticRegressionClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='ovr')
        ...
        self.assertTrue('lr_confs' in configs.keys())
        remove_dir(model_dir)

    def test06_model_logistic_regression_classifier_reload_from_standalone(self):
        '''Test de la fonction ynov.models_training.classifiers.model_logistic_regression_classifier.ModelLogisticRegressionClassifier.reload_from_standalone'''
        ...
        new_model = ModelLogisticRegressionClassifier()
        ...
        self.assertEqual(model.lr.get_params(), lr.get_params())
