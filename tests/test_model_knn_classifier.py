...
from ynov.models_training.classifiers.model_knn_classifier import ModelKNNClassifier
...
class ModelKNNClassifierTests(unittest.TestCase):
    '''Main class to test model_knn_classifier'''
    ...
    def test01_model_knn_classifier_init(self):
        '''Test de ynov.models_training.classifiers.model_knn_classifier.ModelKNNClassifier.__init__'''

        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)

        # Init., test all params
        model = ModelKNNClassifier(model_dir=model_dir)
        ...
        # Check KNN params en fonction multi-label & multiclass strategy
        model = ModelKNNClassifier(model_dir=model_dir, multi_label=False, knn_params={'n_neighbors': 9, 'weights': 'distance'})
        self.assertEqual(model.pipeline['knn'].n_neighbors, 9)
        self.assertEqual(model.pipeline['knn'].weights, 'distance')
        remove_dir(model_dir)
        model = ModelKNNClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='ovr', knn_params={'n_neighbors': 9, 'weights': 'distance'})
        self.assertEqual(model.multiclass_strategy, 'ovr')
        self.assertEqual(model.pipeline['knn'].estimator.n_neighbors, 9)
        self.assertEqual(model.pipeline['knn'].estimator.weights, 'distance')
        remove_dir(model_dir)
        model = ModelKNNClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='ovo', knn_params={'n_neighbors': 9, 'weights': 'distance'})
        self.assertEqual(model.multiclass_strategy, 'ovo')
        self.assertEqual(model.pipeline['knn'].estimator.n_neighbors, 9)
        self.assertEqual(model.pipeline['knn'].estimator.weights, 'distance')
        remove_dir(model_dir)
        #
        model = ModelKNNClassifier(model_dir=model_dir, multi_label=True, knn_params={'n_neighbors': 9, 'weights': 'distance'})
        self.assertEqual(model.pipeline['knn'].n_neighbors, 9)
        self.assertEqual(model.pipeline['knn'].weights, 'distance')
        self.assertEqual(model.multi_label, True)
        remove_dir(model_dir)
        model = ModelKNNClassifier(model_dir=model_dir, multi_label=True, multiclass_strategy='ovr', knn_params={'n_neighbors': 9, 'weights': 'distance'})
        self.assertEqual(model.multiclass_strategy, 'ovr')
        self.assertEqual(model.pipeline['knn'].n_neighbors, 9)
        self.assertEqual(model.pipeline['knn'].weights, 'distance')
        self.assertEqual(model.multi_label, True)
        remove_dir(model_dir)
        model = ModelKNNClassifier(model_dir=model_dir, multi_label=True, multiclass_strategy='ovo', knn_params={'n_neighbors': 9, 'weights': 'distance'})
        self.assertEqual(model.multiclass_strategy, 'ovo')
        self.assertEqual(model.pipeline['knn'].n_neighbors, 9)
        self.assertEqual(model.pipeline['knn'].weights, 'distance')
        self.assertEqual(model.multi_label, True)
        ...
        with self.assertRaises(ValueError):
            model = ModelKNNClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='toto', knn_params={'n_neighbors': 9, 'weights': 'distance'})
        remove_dir(model_dir)

    def test02_model_knn_classifier_predict(self):
        '''Test de la fonction predict de ynov.models_training.classifiers.model_knn_classifier.ModelKNNClassifier'''
        ...
        model = ModelKNNClassifier(x_col=x_col, y_col=y_col_mono, model_dir=model_dir)
        ...
        remove_dir(model_dir)
        model = ModelKNNClassifier(x_col=x_col, y_col=y_col_mono, model_dir=model_dir, multiclass_strategy='ovr')
        model.fit(x_train, y_train_mono_2)
        preds = model.predict(x_train, return_proba=False)
        self.assertEqual(preds.shape, (len(x_train),))
        proba = model.predict(x_train, return_proba=True)
        self.assertEqual(proba.shape, (len(x_train), 2)) # 2 classes
        remove_dir(model_dir)
        model = ModelKNNClassifier(x_col=x_col, y_col=y_col_mono, model_dir=model_dir, multiclass_strategy='ovo')
        model.fit(x_train, y_train_mono_2)
        preds = model.predict(x_train, return_proba=False)
        self.assertEqual(preds.shape, (len(x_train),))
        proba = model.predict(x_train, return_proba=True)
        ...
        model = ModelKNNClassifier(x_col=x_col, y_col=y_col_multi, model_dir=model_dir, multi_label=True)
        ...
        remove_dir(model_dir)
        model = ModelKNNClassifier(x_col=x_col, y_col=y_col_multi, model_dir=model_dir, multi_label=True, multiclass_strategy='ovr')
        model.fit(x_train, y_train_multi)
        preds = model.predict(x_train)
        self.assertEqual(preds.shape, (len(x_train), len(y_col_multi)))
        proba = model.predict(x_train, return_proba=True)
        self.assertEqual(proba.shape, (len(x_train), len(y_col_multi)))
        remove_dir(model_dir)
        model = ModelKNNClassifier(x_col=x_col, y_col=y_col_multi, model_dir=model_dir, multi_label=True, multiclass_strategy='ovo')
        model.fit(x_train, y_train_multi)
        preds = model.predict(x_train)
        self.assertEqual(preds.shape, (len(x_train), len(y_col_multi)))
        proba = model.predict(x_train, return_proba=True)
    ...
    def test03_model_knn_classifier_predict_proba(self):
        '''Test de la fonction predict_proba de ynov.models_training.classifiers.model_knn_classifier.ModelKNNClassifier'''
    ...
    def test04_model_knn_classifier_get_predict_position(self):
        '''Test de la fonction ynov.models_training.classifiers.model_knn_classifier.ModelKNNClassifier.get_predict_position'''
    ...
    def test05_model_knn_classifier_save(self):
        '''Test de la fonction save de ynov.models_training.classifiers.model_knn_classifier.ModelKNNClassifier'''

        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)

        # Fonctionnement nominal
        model = ModelKNNClassifier(model_dir=model_dir, multi_label=False, multiclass_strategy='ovr')
        ...
        self.assertTrue('knn_confs' in configs.keys())
        remove_dir(model_dir)

    def test06_model_knn_classifier_reload_from_standalone(self):
        '''Test de la fonction ynov.models_training.classifiers.model_knn_classifier.ModelKNNClassifier.reload_from_standalone'''
        ...
        self.assertEqual(model.knn.get_params(), knn.get_params())
