...
from ynov.models_training.regressors.model_elasticnet_regressor import ModelElasticNetRegressor
...
class ModelElasticNetRegressorTests(unittest.TestCase):
    '''Main class to test model_elasticnet_regressor'''
    ...
    def test01_model_elasticnet_regressor_init(self):
        '''Test de l'initialisation de ynov.models_training.model_elasticnet_regressor.ModelElasticNetRegressor'''

        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)

        # Init., test all params
        model = ModelElasticNetRegressor(model_dir=model_dir)
        ...
        # Check ElasticNet params
        model = ModelElasticNetRegressor(model_dir=model_dir, elasticnet_params={'alpha': 0.5, 'l1_ratio': 0.8})
        self.assertEqual(model.pipeline['elasticnet'].alpha, 0.5)
        self.assertEqual(model.pipeline['elasticnet'].l1_ratio, 0.8)
        remove_dir(model_dir)

    def test02_model_elasticnet_regressor_predict(self):
        '''Test de la fonction predict de ynov.models_training.model_elasticnet_regressor.ModelElasticNetRegressor'''
        ...
        # Regressor
        model = ModelElasticNetRegressor(x_col=x_col, y_col=y_col_mono, model_dir=model_dir)
    ...
    def test03_model_elasticnet_regressor_save(self):
        '''Test de la fonction save de ynov.models_training.model_elasticnet_regressor.ModelElasticNetRegressor'''
        model_dir = os.path.join(os.getcwd(), 'model_test_123456789')
        remove_dir(model_dir)
        ...
        self.assertTrue('elasticnet_confs' in configs.keys())
        remove_dir(model_dir)

    def test04_model_elasticnet_regressor_reload_from_standalone(self):
        '''Test de la fonction ynov.models_training.regressors.model_elasticnet_regressor.ModelElasticNetRegressor.reload_from_standalone'''
        ...
        new_model = ModelElasticNetRegressor()
        ...
        self.assertEqual(model.elasticnet.get_params(), elasticnet.get_params())
